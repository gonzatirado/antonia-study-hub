...
import {
  ...
  AlertCircle, GraduationCap, ArrowRightLeft, X,
} from "lucide-react";
            ...
            {movingFile && (
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setMovingFile(null)} className="p-1 rounded-md hover:bg-slate-700 text-slate-400 hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {subject.folders
                    ...
                    .map((folder) => (
                      <button
                        key={folder.id}
                        onClick={() => handleMoveFile(movingFile, folder.id)}
                        className="flex items-center gap-2 px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white hover:border-violet-500 hover:bg-slate-700 transition-all"
                      >
